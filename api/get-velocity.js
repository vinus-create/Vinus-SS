...
export default async function handler(req,res){
  ...
  const{shopid,limit=500,from,to,type='variant'}=req.query;
  try{
    // Custom date range — call RPC (variant only)
    if(from&&to){
    ...
    // Product velocity — ALL products, historical_sold delta (no item/get needed)
    if(type==='product'){
      let url=`${S}/rest/v1/product_velocity?order=sold_1d.desc,sold_3d.desc,sold_total.desc&limit=${limit}`;
      if(shopid)url+=`&shopid=eq.${shopid}`;
      const r=await fetch(url,{headers:H});
      return res.status(200).json(await r.json());
    }
    // Default: variant velocity (pre-computed view, enriched products only)
